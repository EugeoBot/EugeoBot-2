...
async function storeReservation(date, doctor) {
...
(async () => {
  availableDoctors = await fetchData();
})();
...
class User {
  ...
  async onIntent(input) {
    ...
    if (this.nextIntent) {
      ...
      console.log(`nextintent ${this.nextIntent}`);
    ...
    if (output.intent === "user.book") {
      if (output.entities) {
        ...
        for (let i = 0; i < output.entities.length; i++) {
          console.log(
            `entitity: ${output.entities[i].entity} option ${output.entities[i].option}`
          );
          switch (output.entities[i].entity) {
            ...
            case "datetime":
              ...
              reservationDatetime =
                this.constructor.processDate(reservationDatetime);
              this.datetime = reservationDatetime;
              break;
            case "time":
              let reservationTime = output.entities[i].sourceText;
              reservationTime = this.constructor.processDate(
                reservationTime,
                true
              );
      ...
      if (this.doctor && (this.datetime || (this.date && this.time))) {
        ...
        if (!this.doctor) {
          ...
          for (let i = 0; i < Object.keys(availableDoctors).length; i++) {
            outputString +=
              availableDoctors[Object.keys(availableDoctors)[i]].name + "\n";
          }
